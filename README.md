...
## Overview
`nxf-emu` is a workflow that processes long-read sequencing data to estimate taxonomic abundance using [Emu](https://github.com/treangenlab/emu). It supports optionally running basecalling with Dorado, read filtering, and generates an interactive HTML report.

## Key Features
- **Basecalling**: Integrated Dorado support for POD5 data.
- **Taxonomy**: High-accuracy species-level profiling with Emu.
...
```bash
nextflow run angelovangel/nxf-emu --reads '/path/to/fastqs'
# --reads can also be a folder with bam files
```

### With Dorado basecalling (POD5 input)
```bash
nextflow run angelovangel/nxf-emu --pod5 '/path/to/pod5s' --kit 'SQK-16S114-24' --samplesheet 'samplesheet.csv'
